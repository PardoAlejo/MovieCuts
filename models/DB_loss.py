...
class ResampleLoss(nn.Module):
    ...
    def __init__(self,
        ...
        classes = self.train_json['cut_types']
        self.num_classes = len(classes)
        self.pos_frequencies = np.zeros(self.num_classes)
        self.neg_frequencies = np.zeros(self.num_classes)
    ...
    def generate_frequencies(self):
        ...
        for clip,info in self.train_json['annotations'].items():
            label = info['labels']
            label_idx_pos = np.nonzero(label)[0]
            label_idx_neg = np.where(np.array(label)==0)
